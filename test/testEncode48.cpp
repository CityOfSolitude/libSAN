...
std::set<uint64_t> createPatterns() {
...
const std::set<uint64_t> patterns = createPatterns(); // NOLINT(cert-err58-cpp)
...
TEST(testEncode48, invalidEmpty) {
    ASSERT_EQ(ERROR_48_EMPTY, decode48(""));
}

TEST(testEncode48, invalidHighBit) {
    std::string encoded;
    for (auto i = -1l; i < 63; ++i) {
        encoded = encode48(i);
        encoded.front() |= static_cast<char>(0x80);
        ASSERT_EQ(ERROR_48_HIGH_BIT, decode48(encoded));
...
TEST(testEncode48, invalidWrongChar) {
    size_t error_high = 0;
    size_t error_wrong = 0;
    size_t success = 0;
    for (int i = -128; i <= 127; ++i) {
        uint64_t decoded = decode48({static_cast<char>(i)});
        switch (decoded) {
            case ERROR_48_HIGH_BIT:
                ++error_high;
                break;
            case ERROR_48_WRONG_CHAR:
                ++error_wrong;
                break;
            case ERROR_48_EMPTY:
                FAIL();
